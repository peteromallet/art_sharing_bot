...
async def get_channel_messages_past_24_hours(channel: discord.TextChannel) -> list[discord.Message]:
    ...
    last_21pm_utc = (current_datetime_utc - timedelta(hours=24)).replace(hour=21, minute=0, second=0, microsecond=0)
...
def replace_user_mentions_with_usernames(message: discord.Message) -> str:
    ...
    for mention_str in mentions_str:
        mention  = [m for m in message.mentions if mention_str == f"<@{m.id}>" ][0]
...
def get_messages_with_most_reactions(messages_with_reaction_counts: list[MessageWithReactionCount], top_n: int) -> list[MessageWithReactionCount]:
    ...
    top_n_reaction_counts = [msg.unique_reactions_count for msg in sorted(messages_with_reaction_counts, key=lambda x: x.unique_reactions_count,reverse=True)][:top_n]
    
    # filter out messages with unique_reactions_count not in top_n_reaction_counts
    top_messages = [msg for msg in messages_with_reaction_counts if msg.unique_reactions_count in top_n_reaction_counts]
    top_messages = sorted(top_messages, key=lambda x: x.unique_reactions_count,reverse=True)
    return top_messages
    ...
    for message in messages:
        ...
        if len(message.attachments) > 0 and len(message.reactions) > 0:
            ...
            if attachment.filename.lower().endswith(('.mp4', '.mov', '.wmv', '.avi', '.gif', '.webm', '.mkv')): 
                ...
                valid_messages_with_reaction_count.append(MessageWithReactionCount(message=message, unique_reactions_count=unique_reactors_count)) 
