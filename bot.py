...
async def on_ready():
    ...
    top_messages = get_messages_with_most_reactions(valid_messages_with_attachments_and_reactions, top_n=4) # select top 4
    
    # post updates to project_art_sharing_channel
    project_art_sharing_channel = client.get_channel(PROJECT_ART_SHARING_CHANNEL)
    ...
    for idx in range(1, len(top_messages)+1):
        ...
        top_message_content = replace_user_mentions_with_usernames(top_message.message)
